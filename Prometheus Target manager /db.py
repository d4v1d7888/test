...
def create_group(data: dict, user_ip: str = None, username: str = None) -> int:
    """
...
def update_group(group_id: int, data: dict, user_ip: str = None, username: str = None) -> bool:
        ...
        with conn.cursor() as cur:
            try:
                ...
                if fields:
                    ...
                    cur.execute(f"UPDATE target_groups SET {', '.join(fields)} WHERE id = %s", params)
                ...
                _log(cur, source_file, "UPDATE", f"Grupo id={group_id} modificado", user_ip, username)
                conn.commit()
                return True
            except Exception:
                conn.rollback()
...
def delete_group(group_id: int, user_ip: str = None, username: str = None) -> bool:
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            try:
                ...
                _purge_orphan_catalogs(cur)
                _log(cur, row["source_file"], "DELETE",
                     f"Eliminado grupo id={group_id} jobname='{row['jobname']}'", user_ip, username)
                conn.commit()
                return True
            except Exception:
                conn.rollback()
...
def add_targets(group_id: int, addresses: list, user_ip: str = None, username: str = None) -> list:
        ...
        with conn.cursor() as cur:
            try:
                ...
                _log(cur, row["source_file"], "UPDATE",
                     f"Añadidos {len(addresses)} targets al grupo id={group_id}", user_ip, username)
                conn.commit()
                return new_ids
            except Exception:
                conn.rollback()
...
def update_target(target_id: int, address: str, user_ip: str = None, username: str = None) -> bool:
        ...
        with conn.cursor() as cur:
            try:
                cur.execute(
                    ...
                    "JOIN target_groups tg ON tg.id = t.target_group_id WHERE t.id = %s",
                    (target_id,),
                )
                row = cur.fetchone()
                if not row:
                ...
                _log(cur, row["source_file"], "UPDATE",
                     f"Target id={target_id} modificado a '{address}'", user_ip, username)
                conn.commit()
                return True
            except Exception:
                conn.rollback()
...
def delete_target(target_id: int, user_ip: str = None, username: str = None) -> bool:
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            try:
                ...
                _log(cur, row["source_file"], "DELETE",
                     f"Eliminado target id={target_id} '{row['address']}'", user_ip, username)
                conn.commit()
                return True
            except Exception:
                conn.rollback()
...
def log_sync_export(source_file: str, user_ip: str = None, username: str = None):
    """Registra en el log que se ha exportado (regenerado) un fichero JSON."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            _log(cur, source_file, "EXPORT",
                 f"Fichero {source_file} regenerado", user_ip, username)
            conn.commit()
...
def _purge_orphan_catalogs(cur):
    """
    Elimina registros de catálogos que ya no están referenciados
    por ningún target_group. Se llama dentro de una transacción activa
    tras eliminar grupos.
    NOTA: git_file_projects NO se limpia aquí — se limpia explícitamente
    en delete_file DESPUÉS del push git, para que _resolve_project pueda
    encontrar la rama correcta durante el push.
    """
    cur.execute(
        "DELETE FROM environments WHERE id NOT IN "
        "(SELECT DISTINCT environment_id FROM target_groups WHERE environment_id IS NOT NULL)"
    )
    cur.execute(
        "DELETE FROM datacenters WHERE id NOT IN "
        "(SELECT DISTINCT datacenter_id FROM target_groups WHERE datacenter_id IS NOT NULL)"
    )
    cur.execute(
        "DELETE FROM os_types WHERE id NOT IN "
        "(SELECT DISTINCT os_id FROM target_groups WHERE os_id IS NOT NULL)"
    )
    cur.execute(
        "DELETE FROM exporters WHERE id NOT IN "
        "(SELECT DISTINCT exporter_id FROM target_groups WHERE exporter_id IS NOT NULL)"
    )


def _log(cur, source_file: str, action: str, detail: str, user_ip: str = None, username: str = None):
    """Inserta una entrada en sync_log (usa el cursor activo para respetar la transacción)."""
    cur.execute(
        "INSERT INTO sync_log (source_file, action, detail, user_ip, username) VALUES (%s,%s,%s,%s,%s)",
        (source_file, action, detail, user_ip, username),
    )
...
def list_users() -> list:
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, username, role, enabled, last_login, created_at "
                "FROM users ORDER BY id"
            )
            return cur.fetchall()


def create_user(username: str, password: str, role: str = "readonly") -> int:
    """Crea un nuevo usuario. Devuelve el id creado."""
    import bcrypt as _bcrypt
    if role not in ("admin", "readonly"):
        raise ValueError("Rol no válido: debe ser 'admin' o 'readonly'")
